const els = {
  ...
  networkDirections: document.querySelector("#network-directions"),
...
function renderNetworkDirections(rows) {
  const max = Math.max(1, ...rows.map((item) => item.count));
  els.networkDirections.innerHTML = rows.map((item) => `
    <div class="list-item">
      <div class="row tight">
  ...
  `).join("") || `<div class="empty">No traffic classified yet. Generate test traffic from Kali.</div>`;
}
...
async function refresh() {
  try {
    ...
    const [metrics, alerts, ollamaReports, reviews, allowlist, enrichment, events, detail, pcaps, evidence, directions] = await Promise.all([
      getJson("/api/metrics"),
      ...
      getJson("/api/network-directions")
    ]);
    ...
    renderNetworkDirections(directions);
    renderEvents(events);
