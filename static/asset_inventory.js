const els = {
  ...
  internalInterfaceCount: document.querySelector("#asset-interface-count"),
...
function renderTypeChart(payload) {
  ...
  renderBars(els.typeChart, rows, (row) => row.name, (row) => row.count, "No device types registered yet.");
}

function renderScoreChart(payload) {
  const rows = payload.by_score || [];
  renderBars(els.scoreChart, rows, (row) => `Score ${row.asset_score}`, (row) => row.count, "No active scores yet.");
}
...
function inventorySearchText(asset) {
  return [
    asset.name,
    asset.ip_address,
    asset.device_type,
...
function filteredAssets() {
  ...
  return (currentPayload.assets || []).filter((asset) => {
    ...
    const queryOk = !query || inventorySearchText(asset).includes(query);
...
function renderRecords() {
  ...
  els.records.innerHTML = assets.map((asset) => `
    <article class="asset-inventory-item ${asset.status === "inactive" ? "inactive" : "active"}">
      ...
      <div class="asset-context-grid">
        ...
        <div>
          ...
          <small>Python adds this score before asking the selected AI profile for review.</small>
        </div>
      </div>
      ${renderRecentDetections(asset)}
    </article>
  `).join("") || `<div class="empty">No inventory records match this filter.</div>`;
}
...
function render(payload) {
  ...
  els.internalInterfaceCount.textContent = summary.internal_interface_count || 0;
...
async function refresh() {
  try {
    render(await getJson("/api/asset-inventory?limit=500"));
  } catch (error) {
    els.updated.textContent = "Asset Inventory API error";
