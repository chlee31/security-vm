...
from app.network import classify_direction


def normalize_suricata_event(event, config=None):
    ...
    normalized = {
        ...
        "capture_iface": event.get("in_iface"),
        "raw_json": json.dumps(event, separators=(",", ":")),
    }

    normalized["direction"] = (
        classify_direction(normalized, config) if config else "unknown"
    )
    return normalized
