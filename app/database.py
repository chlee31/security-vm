"""Own the SQLite schema operations and shared queries used by every worker.

Functions in this module receive an SQLite connection plus ordinary Python
values, then insert, update, correlate, or return rows. Sensor workers write
evidence here, AI workers find cases with missing reports here, and FastAPI read
routes use these query functions to build dashboard JSON. SQLite is therefore
the authoritative local evidence store and the handoff point between processes.

The schema separates four concerns so each stage can be verified:
...
forward-only and preserve existing data when the application starts.
"""
