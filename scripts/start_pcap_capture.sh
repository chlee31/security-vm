...
if [[ "$EUID" -eq 0 ]]; then
  ...
  echo "[+] PCAP capture needs one sudo authentication"
  sudo -v
  PRIVILEGED=(sudo -n)
fi

"${PRIVILEGED[@]}" mkdir -p "$OUTPUT_DIR"
if command -v setfacl >/dev/null 2>&1; then
  "${PRIVILEGED[@]}" setfacl -m "u:${APP_USER}:rx,m:rx" "$OUTPUT_DIR"
  "${PRIVILEGED[@]}" setfacl -d -m "u:${APP_USER}:r,m:r" "$OUTPUT_DIR"
fi
...
cleanup() {
  if [[ -n "${ACL_PID:-}" ]]; then
    "${PRIVILEGED[@]}" kill "$ACL_PID" 2>/dev/null || true
  fi
}
...
start_capture() {
  ...
  "${PRIVILEGED[@]}" dumpcap -i "$interface" \
    -b duration:600 \
    -b filesize:100000 \
    -b files:288 \
